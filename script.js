...
async function submitJsonForm(event, path, after, method = 'POST') {
  event.preventDefault();
  try {
    await api(path, { method, body: JSON.stringify(Object.fromEntries(new FormData(event.target))) });
    event.target.reset();
    await after();
  } catch (error) {
    alert(error.message);
...
async function submitFormData(event, path, after) {
  event.preventDefault();
  try {
    await api(path, { method: 'POST', body: new FormData(event.target) });
    event.target.reset();
    await after();
  } catch (error) {
    alert(error.message);
...
function bindDelegatedActions() {
  document.addEventListener('click', async (event) => {
    ...
    if (event.target.closest('.photo') && photoCard) await openLightbox(Number(photoCard.dataset.id));
